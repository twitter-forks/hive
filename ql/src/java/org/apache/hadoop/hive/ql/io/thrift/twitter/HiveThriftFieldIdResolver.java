    ...
    implements ThriftFieldIdResolver
{
  ...
  public short getThriftId(int hiveIndex)
  {
    ...
    if (thriftId != null) {
      ...
      if (child == null)
        throw new IllegalArgumentException("Missed json value for hiveIndex: " + hiveIndex + ", root: " + root);
      if (child.isNumber()) {
        thriftId = (short) child.asInt();
      }
      else {
        if (child.get("id") == null)
          throw new IllegalArgumentException("Missed id for hiveIndex: " + hiveIndex + ", root: " + root);
